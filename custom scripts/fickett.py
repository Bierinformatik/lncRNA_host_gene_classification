...
position_param  =[1.9,1.8,1.7,1.6,1.5,1.4,1.3,1.2,1.1,0.0]
...
content_param  =[0.33,0.31,0.29,0.27,0.25,0.23,0.21,0.17,0]

def look_up_position(value, base):
	#to look up positional probability by base and value
	if float(value)<0:
		return None
	for idx,val in enumerate (position_param):
		if (float(value) >= val):
			return float(position_prob[base][idx]) * float(position_weight[base])

def look_up_content_prob(value, base):
	#to look up content probability by base and value
	if float(value)<0:
		return None
	for idx,val in enumerate (content_param):
		if (float(value) >= val):
